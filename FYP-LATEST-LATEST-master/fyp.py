...
class SignUp(tk.Toplevel):
    ...
    def click_submit(self):
        ...
        for i in range(2, next_row):
            ...
            self.master.load_test_window()  # Load Test window inside the same window
    ...
    def click_ok(self):
        ...
        for i in range(2, next_row):
            if sheet.cell(row=i, column=1).value == user and sheet.cell(row=i, column=2).value == password:
                found = True
                utils.user = user
                self.load_test_window()  # Open Test window in the same window
                break
        ...
        if not found:
            messagebox.showinfo("Sign In error", "User does not exist! Please Sign Up!")

    def load_test_window(self):
        """Clears the current window and loads the Test UI inside it."""
        for widget in self.master.winfo_children():
            widget.destroy()  # Remove all existing widgets
        
        test_ui = Test(self.master)  # Pass the existing master window
        test_ui.pack(fill=BOTH, expand=True)
